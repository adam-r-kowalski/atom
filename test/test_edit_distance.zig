...
test "edit distance of transposition" {
    ...
    const distance = try mantis.edit_distance.editDistance(allocator, "test", "tset");
    try std.testing.expectEqual(distance, 1);
}

test "edit distance have multiple different characters" {
    const allocator = std.testing.allocator;
    const distance = try mantis.edit_distance.editDistance(allocator, "test", "asdf");
    try std.testing.expectEqual(distance, 4);
}

test "edit distance identical strings" {
    const allocator = std.testing.allocator;
    const distance = try mantis.edit_distance.editDistance(allocator, "test", "test");
    try std.testing.expectEqual(distance, 0);
}

test "edit distance right string is empty" {
    const allocator = std.testing.allocator;
    const distance = try mantis.edit_distance.editDistance(allocator, "test", "");
    try std.testing.expectEqual(distance, 4);
}
...
test "sort by edit distance" {
    const allocator = std.testing.allocator;
    const sorted = try mantis.edit_distance.sort(allocator, "banna", &.{
        "baner",
        "apple",
        "banana",
    });
