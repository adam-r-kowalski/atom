...
test "type infer add i32" {
    ...
    const actual = try zap.testing.typeInfer(allocator, source, "add");
    defer allocator.free(actual);
    const expected =
        \\define =
...
test "type infer binary op multiply" {
    ...
    const actual = try zap.testing.typeInfer(allocator, source, "multiply");
    defer allocator.free(actual);
    const expected =
        \\define =
...
test "type infer divide i32" {
    ...
    const actual = try zap.testing.typeInfer(allocator, source, "div");
    defer allocator.free(actual);
    const expected =
        \\define =
